...
def main(gene_csv_path, gene_list_path, output_path):
    ...
    genes_df = pd.read_csv(
        gene_csv_path, sep='\t', names=['gene_id', 'sequence']
    )
...
if __name__ == "__main__":
    ...
    # Add arguments
    parser.add_argument('-i', '--input', required=True, type=validate_dir, 
                        help='Genes sequences in csv format')
    parser.add_argument('-g', '--genes', required=True, type=validate_dir, 
                        help='Path to gene ids list')
